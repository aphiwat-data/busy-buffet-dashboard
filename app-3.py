...
st.info("""
จากข้อมูลพบว่ามีลูกค้าบางส่วนที่รอแล้ว walk away แสดงให้เห็นว่าเวลารอคิวมีผลต่อพฤติกรรมลูกค้า 
ถ้ารอนานเกินไปอาจทำให้ลูกค้าตัดสินใจออกจากแถวหรือไม่ใช้บริการได้ 
แต่จำนวนคนที่ walk away ก็ไม่ได้สูงมากขนาดนั้น (ปัญหาเกิดจากช่วง peak period ไม่ได้เป็นปัญหาทุกวัน)

**Verdict: Comment 1 = TRUE ✅**
""")
st.divider()
...
labels_day = [d.strftime("%d %b") for d in pax_by_day.index]
colors_day = ["#EF5350" if wa > 5 else "#FFA726" if wa > 0 else "#42A5F5" for wa in wa_by_day.values]

fig, ax = plt.subplots(figsize=(9, 4))
ax.bar(labels_day, pax_by_day.values, color=colors_day, width=0.5, edgecolor="white")
...
st.info("""
จากข้อมูลพบว่าปัญหาเกิดขึ้นเฉพาะวันเสาร์-อาทิตย์เท่านั้น 
โดยวันอาทิตย์ (15 Mar) มีลูกค้าสูงสุด 166 คน และมี walk-away 13 กลุ่ม 
ส่วนวันเสาร์ (14 Mar) มีลูกค้า 154 คน walk-away เพียงกลุ่มเดียว 
ในขณะที่วันธรรมดาทั้งหมดไม่มีคิวเลย 
ดังนั้น ถ้าจัดการ weekend ได้ดีขึ้น ปัญหาส่วนใหญ่น่าจะหมดไปเอง ธุรกิจนี้ยังไปต่อได้

**Verdict: Comment 2 = FALSE ❌** (เกินจริงไปหน่อย walk-away ไม่ได้เยอะจนธุรกิจไปต่อไม่ได้)
""")
st.divider()
...
meal_by_day.index = [d.strftime("%d %b") for d in meal_by_day.index]
...
st.info("""
จากข้อมูลพบว่า Walk-in นั่งเฉลี่ยประมาณ 1 ชั่วโมง ไม่ได้นั่ง "ทั้งวัน" อย่างที่ staff พูด 
อย่างไรก็ตาม Walk-in ยังนั่งนานกว่า In-house อยู่ดี และปัญหาโต๊ะไม่พอเกิดขึ้นจริงในวัน weekend 
ดังนั้น ปัญหาที่แท้จริงอาจไม่ใช่การนั่งนานเกินไป แต่เป็นเรื่องจำนวนลูกค้าที่เพิ่มขึ้นใน weekend มากกว่า

**Verdict: Comment 3 = PARTIALLY TRUE ⚠️**
นั่งนานกว่า In-house จริง แต่ก็ไม่ได้นั่งทั้งวันเหมือนที่ staff พูด
""")
st.divider()
...
counts_bin = pd.cut(meal_data, bins=bins, labels=labels_bin).value_counts().sort_index()
pct = counts_bin / counts_bin.sum() * 100

fig, ax = plt.subplots(figsize=(8, 4))
bars = ax.bar(labels_bin, pct.values, 
              color=["#42A5F5","#FFA726","#EF5350","#B71C1C"], 
...
st.error("""
ปัญหาไม่ได้เกิดจากลูกค้านั่งนานเกินกฎ แต่เกิดจากลูกค้ามาพร้อมกันมากเกินไปในช่วง weekend 
จนโต๊ะไม่พอรองรับ การลดเวลาจึงไม่ได้แก้ปัญหาอะไรเลย
...
st.error("""
ขึ้นราคา 259 ทุกวันไม่ได้แก้ปัญหาคิว weekend เพราะคนที่อยากมา weekend ก็ยังมาอยู่ดี 
แถมยังทำให้ลูกค้าวันธรรมดาหายไปด้วย รายได้เพิ่มจริงแต่แก้ไม่ตรงจุด
...
ax.set_title("Action 3: Who are we actually serving?", fontweight="bold", fontsize=13)
...
st.error("""
ส่วนตัวคิดว่าไม่ใช่การแก้ปัญหาที่ตรงจุด 
ถึงจะโดนเลื่อนให้ In-house ก่อน Walk-in ก็ยังต้องรออยู่ดี 
รวมถึงหากรอนานมากอาจจะเสียลูกค้าบางส่วนด้วย
...
st.subheader("Action 1+2: Raise Weekend Price to 259 + 2hr Seating Limit on Weekend")
...
st.success("""
**Action 1:** ปรับราคาเฉพาะ Weekend เป็น 259 บาท
จากข้อมูลทุกวันหยุดจะมีคิวล้นอยู่แล้ว การปรับราคาจะทำให้คิวลดลงเล็กน้อยแต่ยังคงกำไรไว้ได้ 
คิดว่าจะทำให้มีโต๊ะเพียงพอสำหรับลูกค้าทุกคนด้วย

**Action 2:** ลดเวลานั่งเหลือ 2 ชั่วโมงเฉพาะวันหยุด
จากข้อมูล Avg Time ของแต่ละมื้อจะอยู่ที่ 70 นาทีอยู่แล้ว แต่ก็ยังมีลูกค้าบางส่วนที่นั่งเกิน 
การปรับเวลาจะทำให้การจัดการคิวสมูทขึ้นและช่วยให้โต๊ะหมุนได้ไวขึ้น
""")
